...
func (h *HTTPHandlerV2) HandleCRAAssessmentSubmit(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.sendAssessmentResultsEmail(email, answers, jaCount, complianceScore); err != nil {
	...
	go h.sendAssessmentNotification(email, answers, jaCount, complianceScore)

	// Redirect to success page with score and email
	http.Redirect(w, r, fmt.Sprintf("/cra-assessment?success=true&score=%d&total=%d&email=%s", complianceScore, jaCount, email), http.StatusSeeOther)
}

// sendAssessmentResultsEmail sends the assessment results to the user
func (h *HTTPHandlerV2) sendAssessmentResultsEmail(to string, answers map[string]string, jaCount int, score int) error {
	// Use Mailgun if configured, fallback to SMTP
	if h.mailgunService != nil {
		return h.sendAssessmentEmailViaMailgun(to, jaCount, score)
	}
...
func (h *HTTPHandlerV2) sendAssessmentNotification(email string, answers map[string]string, jaCount int, score int) {
	// Use Mailgun if configured
	if h.mailgunService != nil {
		h.sendAssessmentAdminNotificationViaMailgun(email, jaCount, score)
		return
	}
...
// sendAssessmentEmailViaMailgun sends assessment results via Mailgun
func (h *HTTPHandlerV2) sendAssessmentEmailViaMailgun(to string, jaCount int, score int) error {
	reportID := time.Now().Unix() % 100000
...
<body style="margin: 0; padding: 0; width: 100%%%%; background-color: #f8fafc; font-family: 'Inter', Helvetica, Arial, sans-serif; -webkit-font-smoothing: antialiased;">
	...
	subject := fmt.Sprintf("CRA Compliance Briefing - %d%%%% Score", score)
	return h.mailgunService.SendHTMLEmailWithBCC(to, "kim@leonacompliance.be", subject, body)
}

// sendAssessmentAdminNotificationViaMailgun sends admin notification via Mailgun
func (h *HTTPHandlerV2) sendAssessmentAdminNotificationViaMailgun(email string, jaCount int, score int) {
	body := fmt.Sprintf(`
<!DOCTYPE html>
<html>
<head>
...
`, email, score, jaCount, getLeadQuality(score), time.Now().Format("2006-01-02 15:04:05"))
	...
	subject := fmt.Sprintf("Nieuwe CRA Assessment: %s - %d%%%% Score", email, score)
	if err := h.mailgunService.SendHTMLEmail("kim@leonacompliance.be", subject, body); err != nil {
		log.Printf("❌ ERROR: Failed to send admin notification via Mailgun: %v", err)
	} else {
		log.Printf("✅ SUCCESS: CRA assessment notification sent to kim@leonacompliance.be for %s (%d%%%% score)", email, score)
	}
}
